...
const grassMaterial = new THREE.MeshFaceMaterial([
...
const woodMaterial = new THREE.MeshFaceMaterial([
...
const playerMaterial = new THREE.MeshFaceMaterial([
...
function init() {
  ...
  MovingCube = new THREE.Mesh(new THREE.BoxGeometry(BLOCK_SIZE, 16, BLOCK_SIZE), playerMaterial);
...
function changeInventory(invSlot) {
  ...
  document.getElementsByClassName('inv-item')[invSlot - 1].classList.add('current')
}

function generateChunk() {
  for (var x = 0; x < 10; x++) {
    for (var z = 0; z < 10; z++) {
      var voxel = new THREE.Mesh(new THREE.BoxGeometry(BLOCK_SIZE, BLOCK_SIZE, BLOCK_SIZE), grassMaterial);
      voxel.position.x = 25 - x * BLOCK_SIZE + BLOCK_SIZE;
      voxel.position.z = 25 - z * BLOCK_SIZE + BLOCK_SIZE;
      voxel.position.y = BLOCK_SIZE * Math.round(Math.random()) + BLOCK_SIZE / 2;
      scene.add(voxel);
      objects.push(voxel);
  ...
  switch (event.buttons) {
    case 1:
      ...
      if (intersects.length > 0 && intersects[0].distance < BLOCK_SIZE * 3) {
        ...
        collidableMeshList = collidableMeshList.filter(e => e.uuid != intersects[0].object.uuid)
        scene.children.forEach(e => e.uuid == intersects[0].object.uuid ? scene.remove(e) : null)
      }
      break;
    case 2:
      var intersects = raycaster2.intersectObjects(objects);
      if (intersects.length > 0 && intersects[0].distance < BLOCK_SIZE * 3) {
        ...
        var voxel = new THREE.Mesh(new THREE.BoxGeometry(BLOCK_SIZE, BLOCK_SIZE, BLOCK_SIZE), currentMaterial);
