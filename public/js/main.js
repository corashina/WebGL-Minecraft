...
function init() {
  ...
  window.addEventListener("keydown", (e) => {
    if (e.keyCode === 13) {
      if (document.getElementById('chat').style.opacity != 1) {
    ...
    } else if (document.getElementById('chat').style.opacity != 1) {
      keys[e.keyCode] = true
    }
  })
}
...
function generateWorld(world) {
  world.chunks.forEach(chunk => chunk.blocks.forEach(block => {
    let voxel = new THREE.Mesh(new THREE.BoxGeometry(BLOCK_SIZE, BLOCK_SIZE, BLOCK_SIZE), materials[block.materialIndex]);
    voxel.position.x = block.x;
    voxel.position.y = block.y;
    voxel.position.z = block.z;
    voxel.uuid = block.uuid;
    scene.add(voxel);
    objects.push(voxel)
    collidableMeshList.push(voxel)
  }))
}
  ...
  switch (event.buttons) {
    case 1:
      ...
      if (intersects.length > 0 && intersects[0].distance < BLOCK_SIZE * 3) {
        socket.emit('removeBlock', intersects[0]);
      }
      break;
    case 2:
      var intersects = raycaster2.intersectObjects(objects);
      if (intersects.length > 0 && intersects[0].distance < BLOCK_SIZE * 3) {
        socket.emit('addBlock', intersects[0], currentMaterialIndex);
      }
      break;
    default:
      break;
...
function addMessage(message) {
  ...
  document.getElementById('chat-messages').appendChild(text)
  document.getElementById('chat-input').value = '';
  if (document.getElementById('chat-messages').childElementCount == 6) {
    document.getElementById('chat-messages').removeChild(document.getElementById('chat-messages').childNodes[0]);
  }
  setTimeout(() => document.getElementById('chat').style.opacity = 0, 1000)
}
...
function animate() {
  ...
  if (controlsEnabled) {
    ...
    PlayerModel.translateOnAxis(new THREE.Vector3(1, 0, 0), -1.5);
    PlayerModel.translateOnAxis(new THREE.Vector3(0, 0, 1), 6);
...
socket.on('update', (position, rotation) => {
  other.rotation.copy(rotation);
  other.position.copy(position);
  other.position.y -= 13;
})

socket.on('addBlock', (block, material) => addBlock(block, material))
socket.on('removeBlock', (block) => removeBlock(block))
socket.on('world', (world) => generateWorld(JSON.parse(world)))
socket.on('message', (message) => { addMessage(message); })

setInterval(() => {
  socket.emit('request_update', PlayerModel.position, PlayerModel.rotation)
}, 16)
